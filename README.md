...
一套轻量、可扩展、面向中小型制造企业的 MES（制造执行系统）整体解决方案。采用 **WebApi 后端 + 双客户端** 架构，实现从工艺定义、任务下发到现场执行、数据闭环的全生产流程管控。
...
## 项目简介
本项目旨在为中小型制造企业提供一套**开箱即用**的 MES 系统骨架。系统核心设计围绕**模块化**与**可扩展性**展开，通过区分「二线工艺管理」与「一线生产执行」，提供了高度贴合工业现场的差异化解决方案。
...
## 整体架构设计
系统以 **WebApi 后端** 为数据与服务中枢，搭配两大客户端，构建了标准化的三层体系。

### 架构核心分工
| 模块 | 核心定位 | 目标用户 | 核心职能 |
| :--- | :--- | :--- | :--- |
| **WebApi 后端** | 数据/服务中枢 | 开发/运维人员 | 提供标准化 API、权限管控、数据交互 |
